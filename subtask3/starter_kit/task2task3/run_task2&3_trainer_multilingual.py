...
def parser_getting():
    ...
    parser.add_argument('--gpu', type=lambda x: str(x).lower() in ['true', '1', 'yes'], default=True)
...
def train(args, train_total_data, test_total_data, inference_dataset, category_mapping):
    ...
    if args.gpu:
        if torch.cuda.is_available():
            ...
            logger.warning('CUDA requested but not available. Continuing with CPU execution.')
            args.gpu = False
    ...
    if args.mode == 'evaluate':
        ...
        if os.path.exists(model_path):
            ...
            checkpoint = torch.load(model_path, map_location='cuda' if args.gpu and torch.cuda.is_available() else 'cpu')
            model.load_state_dict(checkpoint['net'])
        else:
            logger.warning('No checkpoint found at %s. Using initialised weights for evaluation.', model_path)
    ...
    if args.mode == 'inference':
        ...
        if os.path.exists(model_path):
            ...
            logger.warning('No checkpoint found at %s. Using initialised weights for inference.', model_path)
